...
from models.ColpoCNN import resnet18
from models.inceptionnext import inceptionnext_tiny
from models.ColpoSwinTrans import Swintransformer
...
def weight_init(module):
    for n, m in module.named_children():
        ...
        elif isinstance(m, (nn.GELU,Act,nn.AdaptiveAvgPool2d,nn.Softmax,nn.ReLU,nn.AdaptiveAvgPool1d)):
            pass
        else:
            m.initialize()
...
class Grafting(nn.Module):
    ...
    def forward(self, x, y):
        ...
        max_pool1d_1 = nn.AdaptiveMaxPool1d(150)
...
class deepLEEP(nn.Module):
    def __init__(self, cfg=None):
        super(deepLEEP, self).__init__()
        self.cfg      = cfg
        ...
        self.ssqz_s2=nn.Sequential(
            nn.Conv2d(512, 64, kernel_size=3,stride=1,dilation=1,padding=1), 
            nn.BatchNorm2d(64), 
            nn.ReLU(inplace=True)
            )
        self.ssqz_r5 = nn.Sequential(
    ...
    def forward(self, x,shape=None,mask=None):
        #high resolution of shape 480 x 320 which is input of resnet18
        ...
        s4 = F.interpolate(s4,size = r5.size()[2:],mode='bilinear',align_corners=True)  # R5上采样到S2的尺寸
        s4_ = self.ssqz_s2(s4)
        r5_= self.ssqz_r5(r5)
        graft_feature_r5,_ = self.GF(r5_,s4_)

        out = self.ssqz_end(graft_feature_r5)
